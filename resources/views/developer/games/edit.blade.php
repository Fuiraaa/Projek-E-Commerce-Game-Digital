<x-app-layout>
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            <h1 class="font-heading text-3xl font-bold text-white neon-text">Edit Game</h1>
            <p class="text-gray-400 mt-1">Update game details</p>
        </div>

        <form id="gameForm" action="{{ route('developer.games.update', $game) }}" method="POST" enctype="multipart/form-data" class="glass-card p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $game->title) }}" class="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder-gray-500 focus:border-neon-blue focus:ring-1 focus:ring-neon-blue outline-none transition-colors" required>
                    @error('title')
                ...
                <div>
                    ...
                    <textarea name="description" rows="5" class="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder-gray-500 focus:border-neon-blue focus:ring-1 focus:ring-neon-blue outline-none transition-colors resize-none" required>{{ old('description', $game->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $game->price) }}" min="0" class="w-full px-4 py-3 rounded-lg bg-white/5 border border-white/10 text-white placeholder-gray-500 focus:border-neon-blue focus:ring-1 focus:ring-neon-blue outline-none transition-colors" required>
                    @error('price')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if($game->cover_image)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $game->cover_image) }}" alt="{{ $game->title }}" class="w-32 h-32 object-cover rounded-lg">
                        </div>
                    @endif
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Upload a new .zip to replace current file. Max 200MB.</p>
                    @error('game_file')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-6 flex items-center gap-4">
                <button type="submit" id="submitBtn" class="btn-primary px-6 py-3 rounded-xl text-white font-medium flex items-center gap-2">
                    <span id="btnText">Update Game</span>
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (hasFile) {
                ...
                xhr.upload.addEventListener('progress', function(e) {
                    if (e.lengthComputable) {
                        ...
                        if (percent < 100) {
                            ...
                            uploadStatus.textContent = 'Upload complete! Saving changes...';
                        }
                    }
                });
                ...
                xhr.addEventListener('error', function() {
                    ...
                    btnText.textContent = 'Update Game';
                    btnSpinner.classList.add('hidden');
                });

                xhr.open('POST', '{{ route("developer.games.update", $game) }}?_method=PUT');
